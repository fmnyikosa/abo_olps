...
load(sprintf('OLPS/Data/%s.mat', dataset_name));
if (~opts.quiet_mode)
    fprintf(1, ...
           'Finish loading dataset %s\nThe size of the dataset is %dx%d.\n', ...
...
if (opts.log_mode)
    ...
    file_name = ['OLPS/Log/Log/' strategy_name '-' dataset_name '-' dt '.txt'];
...
if (opts.analyze_mode)
    [ra_ret] = ra_result_analyze(fid, data, cum_ret, cumprod_ret,...
                                                               daily_ret, opts);
end
...
if (opts.mat_mode)
    ...
    mat_name = ['OLPS/Log/Mat/' strategy_name '-' dataset_name '-' mat_dt '.mat'];
    save(mat_name,  'cum_ret', 'cumprod_ret', 'daily_ret', 'ra_ret',...
                                                 'run_time', 'daily_portfolio');
end

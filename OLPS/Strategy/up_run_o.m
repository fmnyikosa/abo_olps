function [cum_ret, cumprod_ret, daily_ret, daily_portfolio] ...
    = up_run_o(fid, data, tc, opts)
% This file is the run core for the up strategy.
%
...
for t = 1:1:n,
    ...
    if (t >= 2)
        [day_weight] = up_kernel(data(1:t-1, :), day_weight_o);
    end
...
% Debug Information
fprintf(fid, 'UP(tc=%.4f), Final return: %.2f\n', tc, cum_ret);
fprintf(fid, '-------------------------------------\n');
fprintf(1,  'UP(tc=%.4f), Final return: %.2f\n', tc, cum_ret);
fprintf(1, '-------------------------------------\n');
